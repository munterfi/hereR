...
#'
#' Calculates route geometries (\code{LINESTRING}) between given pairs of points using the 'Public Transit' API.
#'
#' @references
...
#'   url_only = TRUE
#' )
route_pt <- function(start, destination, results = 3, time = Sys.time(),
                     arrival = FALSE, url_only = FALSE) {
  # Checks
  .check_points(start)
        ...
        data.table::rbindlist(
          lapply(df$Res$Connections$Connection$Sections$Sec, function(sec) {
            ...
            data.table::data.table(
              ...
              dep_station = sec$Dep$Stn$name,
              arr_time = sec$Arr$time,
              arr_station = sec$Arr$Stn$name,
