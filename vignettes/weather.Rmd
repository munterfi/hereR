...
An hourly forecast of the predicted weather for the following seven days can be obtained by setting the `product` parameter to `"forecastHourly"`:
...
An astronomical forecast is requested by setting the `product` parameter to `"forecastAstronomy"`:

```{r astronomy, eval = FALSE}
astronomy <- weather(
  poi = poi,
  product = "forecastAstronomy"
)
```
