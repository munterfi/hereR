...
Request public transport connections between given points and find stations around locations based on the 'HERE Public Transit' API.

## Connections
The function `connection()` allows to request public transport connections from the API. Two types of requests are provided:

* `connection(..., summary = FALSE)`: The public transport connections are returned as mulitple sections with the same vehicle and transport mode. Each row represents a section with a detailed route geometry.
